...
import gc
import cv2
from chainercv.evaluations import eval_instance_segmentation_voc, eval_detection_voc
...
def run_occlusion_analysis_with_classes(p_masks, p_bboxes, p_labels, p_scores, gt_masks, gt_bboxes, gt_labels,
                                        occ_scores, iou_thresh, bins,
                                        bin_labels):
    bin_results = []
    ...
    for (low, high), label in zip(bins, bin_labels):
        ...
        for p_m, p_b, p_l, p_s, gt_m, gt_b, gt_l, gt_o in zip(p_masks, p_bboxes, p_labels, p_scores, gt_masks,
                                                              gt_bboxes, gt_labels, occ_scores):
            ...
            if len(keep_gt_idx) > 0:
                ...
                filtered_gt_bboxes.append(gt_b[keep_gt_idx])
                ...
                filtered_gt_masks.append(
                    np.empty((0, gt_m.shape[1] if gt_m.shape[0] > 0 else 1, gt_m.shape[2] if gt_m.shape[0] > 0 else 1),
                             dtype=bool))
                filtered_gt_bboxes.append(np.empty((0, 4), dtype=np.float32))
                filtered_gt_labels.append(np.array([], dtype=int))
                valid_classes = set()

            if len(p_m) == 0:
                filtered_p_masks.append(p_m)
                filtered_p_bboxes.append(p_b)
            ...
            if len(keep_p) > 0:
                ...
                filtered_p_bboxes.append(p_b[keep_p])
                ...
                filtered_p_bboxes.append(np.empty((0, 4), dtype=np.float32))
                filtered_p_labels.append(np.array([], dtype=int))
                filtered_p_scores.append(np.array([], dtype=np.float32))

        print(f"  -> Evaluating instances with {label} occlusion @ Box IoU {iou_thresh}...")

        # --- ADD THIS CHECK TO PREVENT THE CRASH ---
        total_gt_instances = sum(len(l) for l in filtered_gt_labels)
        if total_gt_instances == 0:
            print(f"     -> Skipping '{label}' bin: 0 ground truth instances found.")
            bin_results.append({
        ...
        res = eval_detection_voc(
            filtered_p_bboxes, filtered_p_labels, filtered_p_scores,
            filtered_gt_bboxes, filtered_gt_labels, iou_thresh=iou_thresh
        )
        ...
        del filtered_gt_masks, filtered_gt_bboxes, filtered_gt_labels, filtered_p_masks, filtered_p_bboxes, filtered_p_labels, filtered_p_scores
        gc.collect()
...
def get_chainercv_format(coco_api, imgIds, is_gt=True, cat2idx=None, max_dets=100, score_thr=0.01):
    all_masks, all_bboxes, all_labels, all_scores = [], [], [], []
    ...
    for img_id in tqdm(imgIds, desc=f"Converting {'GT' if is_gt else 'Pred'} formats"):
        ...
        if len(masks) > 0:
            ...
            all_bboxes.append(np.empty((0, 4), dtype=np.float32))
            all_labels.append(np.empty((0,), dtype=np.int32))
            if not is_gt:
                all_scores.append(np.empty((0,), dtype=np.float32))

    if is_gt:
        return all_masks, all_bboxes, all_labels, cat2idx
    else:
        return all_masks, all_bboxes, all_labels, all_scores
...
if __name__ == '__main__':
    ...
    if "gt" not in parser.dataset:
        if parser.dataset == "voc_val":
            ...
            label_file = "./datasets/coco2017/annotations/image_info_test-dev2017.json"
            root = './datasets/coco2017/test2017'
        ...
        gt_masks, gt_bboxes, gt_labels, cat2idx = get_chainercv_format(cocoGt, imgIds, is_gt=True)
        ...
        for result_file in result_files:
            ...
            p_masks, p_bboxes, p_labels, p_scores = get_chainercv_format(cocoDt1, imgIds, is_gt=False, cat2idx=cat2idx)

            print(f"\n--- Calculating Overall mAP for {model_name} at IoUs: {overall_ious} ---")
            model_overall_mAPs = {}
            for iou in overall_ious:
                res = eval_detection_voc(
                    p_bboxes, p_labels, p_scores,
                    gt_bboxes, gt_labels, iou_thresh=iou
                )
            ...
            for occ_type, occ_scores, target_results_dict, specific_bins, specific_labels in run_configurations:
                ...
                for iou in iou_thresholds:
                    results = run_occlusion_analysis_with_classes(p_masks, p_bboxes, p_labels, p_scores, gt_masks,
                                                                  gt_bboxes, gt_labels,
            ...
            for i in range(len(gt_masks)):
                if len(gt_masks[i]) == 1:
                    ...
                    single_gt_bboxes.append(gt_bboxes[i])
                    single_gt_labels.append(gt_labels[i])
                    single_p_masks.append(p_masks[i])
                    single_p_bboxes.append(p_bboxes[i])
                    single_p_labels.append(p_labels[i])
                    single_p_scores.append(p_scores[i])

            if len(single_gt_masks) > 0:
                for iou in overall_ious:
                    res_single = eval_detection_voc(
                        single_p_bboxes, single_p_labels, single_p_scores,
                        single_gt_bboxes, single_gt_labels, iou_thresh=iou
                    )
