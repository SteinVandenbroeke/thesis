...
def get_model(args):
    model = vgg16_pam(pretrained=True, num_classes=200, alpha=args.alpha) 
...
def train(current_epoch):
    ...
    # Write the epoch and loss to a CSV file
    import os
    log_file = 'checkpoints/PAM/loss_log.csv'
    
    # Add a header if it's the first epoch
    if current_epoch == 1 and not os.path.exists(log_file):
        with open(log_file, 'w') as f:
            f.write("Epoch,Train_Loss\n")
            
    # Append the data
    with open(log_file, 'a') as f:
        f.write(f"{current_epoch},{avg_loss:.4f}\n")
    # ============================================================
