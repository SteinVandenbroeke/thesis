...
def run_occlusion_analysis_with_classes(p_masks, p_labels, p_scores, gt_masks, gt_labels, occ_scores, iou_thresh):
    ...
    for (low, high), label in zip(bins, bin_labels):
        ...
        for m, l, o in zip(gt_masks, gt_labels, occ_scores):
            keep_indices = np.where((o > low) & (o <= high))[0]
            if len(keep_indices) > 0:
                filtered_gt_masks.append(m[keep_indices])
                filtered_gt_labels.append(l[keep_indices])
            else:
                filtered_gt_masks.append(
                    np.empty((0, m.shape[1] if m.shape[0] > 0 else 1, m.shape[2] if m.shape[0] > 0 else 1), dtype=bool))
...
def get_chainercv_format(coco_api, imgIds, is_gt=True, cat2idx=None):
...
def dataset_mask(cocoGT, cocoDt, save_dir, imgIds=None, num=None, anno_thr=-1, name_mapping=None, img_category_map=None):
    if imgIds == None:
        imgIds = sorted(cocoGT.getImgIds())
    ...
    for index in tqdm(range(len(imgIds)), total=len(imgIds)):
        ...
        gt_masks_dict = {} # Pre-calculate GT masks by class id for easy IoU lookup
        if len(gt_anns) > 0:
            ...
            for idx, ann in enumerate(gt_anns):
                ...
                if 'segmentation' in ann:
                    ...
                    # Save the mask array for calculating Pred IoU later
        ...
        if len(anns) > 0:
            ...
            for idx, ann in enumerate(anns):
                ...
                if 'segmentation' in ann:
                    ...
                    # Calculate max IoU for this prediction against any GT of the same class
                    cat_id = ann['category_id']
                    max_iou = 0.0
                    ...
                    # Forge a unique custom class name with the IoU embedded
                    orig_cls_name = name_mapping[Cls_Ids.index(cat_id)]
                    custom_name = f"{orig_cls_name} | IoU:{max_iou:.2f}"
                    
                    dt_polygons.append(mask_t)
                    for _ in range(mask_t.shape[0]):
                        dt_label_list.append(len(dt_custom_names)) # Unique index mapped to custom_name
            ...
            if len(dt_polygons) > 0:
                ...
                # Attach the score to the end of the bounding box so the drawer adds it to the label
...
if __name__ == '__main__':
    ...
    if "gt" not in parser.dataset:
        ...
        # Build reverse mappings to lookup class names from ChainerCV labels
        ...
        for result_file in result_files:
            ...
            # Calculate Overall mAP for specific IoUs (No occlusion filtering)
            ...
            # Store the dataframes for the final combined comparison plot later
            ...
            # Map images to folders based on max occlusion and average image IoU
            img_category_map = {}
            for i, img_id in enumerate(imgIds):
                ...
                # Categorize based on the 0.5 IoU threshold
                if img_iou < 0.5:
            ...
            for worker_id in range(worker):
                if worker_id + 1 != worker:
                    p = multiprocessing.Process(target=dataset_mask,
                                                args=(cocoGt, cocoDt1, masks_dir,
                                                      imgIds[worker_id * per_len:(worker_id + 1) * per_len], parser.num,
                                                      thr, cls_name_map, img_category_map))
                else:
                    p = multiprocessing.Process(target=dataset_mask,
                                                args=(cocoGt, cocoDt1, masks_dir, imgIds[worker_id * per_len:], parser.num, thr, cls_name_map, img_category_map))
                jobs.append(p)
                p.start()
            for p in jobs:
                p.join()
